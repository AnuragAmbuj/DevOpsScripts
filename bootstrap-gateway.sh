...
# ---------- helpers ----------
insert_publish_false() {
  # Insert 'publish = false' into the FIRST [package] table (BSD sed)
  local toml="$1"
  if ! grep -qE '^[[:space:]]*publish[[:space:]]*=[[:space:]]*false[[:space:]]*$' "$toml"; then
    sed -i .bak '0,/\[package\]/{/\[package\]/a\
publish = false
}' "$toml" && rm -f "$toml.bak"
  fi
}
...
mk_lib_crate () {
  # $1 path, $2 optional package name override
  ...
  insert_publish_false "$path/Cargo.toml"
...
# ---------- gateway ----------
# Name the core crate 'gateway-core' to avoid stdlib 'core' warning
