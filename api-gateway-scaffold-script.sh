...
# ---------------- GATEWAY ----------------
declare -a GATEWAY_LIBS=(
  core proxy authn authz limits routing transforms observability snapshot
  plugin-sdk plugin-host tls errors commons
)
...
touch gateway/tests/integration/.keep
echo "# Gateway tests" > gateway/tests/README.md

# ---------------- CONTROL PLANE ----------------
declare -a CP_LIBS=(domain storage events contracts)
for c in "${CP_LIBS[@]}"; do
  mk_lib_crate "control-plane/crates/$c"
  append_workspace_member "control-plane/crates/$c"
done

declare -a CP_SERVICES=(admin-api distributor idp secrets auditor)
...
touch control-plane/migrations/.keep
mkdir -p control-plane/tests
touch control-plane/tests/.keep
...
EOF
touch console/app/.keep console/components/.keep console/features/.keep console/lib/.keep console/e2e/.keep
...
touch plugins/.keep plugins/rust/.keep plugins/wasm/.keep
echo "# Plugins playground" > plugins/README.md

# ---------------- TOOLING / DOCS / CI ----------------
mkdir -p tooling/helm tooling/k6 tooling/devcontainer tooling/scripts
touch tooling/helm/.keep tooling/k6/.keep tooling/devcontainer/.keep tooling/scripts/.keep
...
EOF
touch docs/runbooks/.keep docs/design/.keep
...
tree: ; find . -maxdepth 4 -type d | sort
EOF

# Wire members into Cargo.toml
